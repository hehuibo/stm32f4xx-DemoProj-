...
void AppTaskInit(void){
  ...
  InitBoardPeripheral(); 
#if 1 
  LcdPutStr("液晶显示1", sizeof("液晶显示1"), eLcdWRCtrlLine_One, 0);
  LcdPutStr("液晶显示3", sizeof("液晶显示2"), eLcdWRCtrlLine_Three, 0);
  LcdPutStr("液晶显示2", sizeof("液晶显示3"), eLcdWRCtrlLine_Two, 0);
  LcdPutStr("液晶显示4", sizeof("液晶显示4"), eLcdWRCtrlLine_Four, 0);  
  //vTestSramRDWR();
#endif
...
void CardIssure(void){
  ...
  do{
    ...
    ret = gfn_rfidCtrl.pfnCmdPro(g_AppCommBfrMnt.pTxBfr, g_AppCommBfrMnt.mTxLen, g_AppCommBfrMnt.pRxBfr, (uint8_t *)&g_AppCommBfrMnt.mRxLen);
#else
...
void CardTransference(void){
  ...
  do{
    ...
    /* 过程密钥计算   MAC1 MAC2计算 */
    memcpy(msgIn,&g_AppCommBfrMnt.pRxBfr[10],4); /*伪随机数*/ 
    memcpy(&msgIn[4],&g_AppCommBfrMnt.pRxBfr[6],2);
    memcpy(&msgIn[6],"\x80\x00",2);
    gfn_rfidCtrl.pfnDES(eRFID_ENCRY,msgIn,PassKeyB,PassKeyMac);
    
    memcpy(mac1,"\x00\x00\x00\x00\x00\x00\x00\x00",8);
    memcpy(msgIn,"\x00\x00\x00\x00\x00\x00\x12\x34\x02\x01\x02\x03\x04\x05\x06",15);
    gfn_rfidCtrl.pfnMAC(mac1,PassKeyMac,15,msgIn,msgOut);
    
    memcpy(msgIn,"\x00\x00\x12\x34",4);                                               /* 校验金额00001234 */
    memcpy(&msgIn[4],"\x02",1);                                                       /* 交易类型识别吗   */
    memcpy(&msgIn[5],"\x01\x02\x03\x04\x05\x06",6);                                   /* 交易终端识别码  */
    memcpy(&msgIn[11],"\x20\x16\x05\x05\x03\x04\x05",7);                              /* 交易时间  */
    gfn_rfidCtrl.pfnMAC(mac1,PassKeyMac,18,msgIn,mac2);
    
    /*电子钱包圈存交易*/
    g_AppCommBfrMnt.mTxLen = 12;
    memcpy(&g_AppCommBfrMnt.pTxBfr[2],"\x80\x52\x00\x00\x0b\x20\x16\x05\x05\x03\x04\x05",g_AppCommBfrMnt.mTxLen); /*  圈存指令               */            
    memcpy(&g_AppCommBfrMnt.pTxBfr[2+g_AppCommBfrMnt.mTxLen],mac2, 4);/*  圈存指令 */
    g_AppCommBfrMnt.mTxLen += 4;
    ret = gfn_rfidCtrl.pfnCmdPro(g_AppCommBfrMnt.pTxBfr, g_AppCommBfrMnt.mTxLen, g_AppCommBfrMnt.pRxBfr, (uint8_t *)&g_AppCommBfrMnt.mRxLen);
    if(MI_OK != ret){
      break;
    }
    ...
    /*电子钱包余额*/
    g_AppCommBfrMnt.mTxLen = 5;
    memcpy(&g_AppCommBfrMnt.pTxBfr[2],"\x80\x5C\x00\x02\x04",g_AppCommBfrMnt.mTxLen);       /*  读取余额*/            
    ret = gfn_rfidCtrl.pfnCmdPro(g_AppCommBfrMnt.pTxBfr, g_AppCommBfrMnt.mTxLen, g_AppCommBfrMnt.pRxBfr, (uint8_t *)&g_AppCommBfrMnt.mRxLen);
    if(MI_OK != ret){
      break;
    }
...
void CardDec(void){
  ...
  do{
    ...
    /* 过程密钥计算   MAC1 MAC2计算 */
    memcpy(msgIn,&g_AppCommBfrMnt.pRxBfr[13],4); /*伪随机数*/ 
    memcpy(&msgIn[4],&g_AppCommBfrMnt.pRxBfr[6],2);/*旧余额*/ 
    memcpy(&msgIn[6],"\x00\x00",2);
    gfn_rfidCtrl.pfnTDES(eRFID_ENCRY,msgIn,PurchaseKeyA,PassKeyMac);
    ...
    gfn_rfidCtrl.pfnMAC(mac,PassKeyMac,18,msgIn,mac1);
    
    g_AppCommBfrMnt.mTxLen = 16;
    memcpy(&g_AppCommBfrMnt.pTxBfr[2],"\x80\x54\x01\x00\x0f\x00\x00\x00\x00\x20\x16\x05\x05\x03\x04\x06", g_AppCommBfrMnt.mTxLen);/*消费指令*/
    memcpy(&g_AppCommBfrMnt.pTxBfr[2+g_AppCommBfrMnt.mTxLen],mac1, 4);
    g_AppCommBfrMnt.mTxLen += 4;
    memcpy(&g_AppCommBfrMnt.pTxBfr[2+g_AppCommBfrMnt.mTxLen],"\x08", 1);
    g_AppCommBfrMnt.mTxLen += 1;
    ret = gfn_rfidCtrl.pfnCmdPro(g_AppCommBfrMnt.pTxBfr, g_AppCommBfrMnt.mTxLen, g_AppCommBfrMnt.pRxBfr, (uint8_t *)&g_AppCommBfrMnt.mRxLen);
    #if defined (UART_TRACE) || defined (JLINK_RTT_TRACE)
    dbgTRACE("电子钱包消费交易\n");
    #endif 
...
void vDelay1STask(void){
  ...
  //CardIssure();
#if 1
  LcdPutStr("液晶显示11  ", sizeof("液晶显示11  "), eLcdWRCtrlLine_One, 0);
  LcdPutStr("液晶显示13  ", sizeof("液晶显示12  "), eLcdWRCtrlLine_Three, 0);
  LcdPutStr("液晶显示12  ", sizeof("液晶显示13  "), eLcdWRCtrlLine_Two, 0);
  LcdPutStr("液晶显示14  ", sizeof("液晶显示14  "), eLcdWRCtrlLine_Four, 0); 
#endif
