...
const uint8_t ShiftValue[16]={1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };
...
uint8_t rfidTdes(enum eRFIDCRYMode Mode,uint8_t *MsgIn, uint8_t *Key, uint8_t *MsgOut)
{
  ...
  if(Mode ==eRFID_ENCRY){//加密
    for (i=0;i<8;i++){
      des_key_temp[i]=Key[i];
      Min[i]=MsgIn[i];
    }
    rfidDes(eRFID_ENCRY,Min,des_key_temp,Mout);

    for (i=0;i<8;i++){
      des_key_temp[i]=Key[i+8];
      Min[i]=Mout[i];
    }
    rfidDes(eRFID_DECRY,Min,des_key_temp,Mout);
...
//返 回 值：8
/**************************************************************************************/
uint8_t TripleMAC(uint8_t *init_data,uint8_t *mac_key,uint8_t data_len,uint8_t *in_data,uint8_t *mac_data)
{
  ...
  for(j=0;j<len;j++){
    ...
    rfidDes(eRFID_ENCRY,mac_data0,L,mac_data1);     
    memcpy(mac_data0,mac_data1,8);
  }

  rfidDes(eRFID_DECRY,mac_data0,R,mac_data1);
  memcpy(mac_data0,mac_data1,8);
  rfidDes(eRFID_ENCRY,mac_data0,L,mac_data1);
...
unsigned char rfidMAC(unsigned char *init_data,unsigned char *mac_key,unsigned char data_len,unsigned char *in_data,unsigned char *mac_data)
{
  ...
  for(j=0;j<len;j++){
    ...
    rfidDes(eRFID_ENCRY,mac_data0,mac_key,mac_data1);     
    memcpy(mac_data0,mac_data1,8);
  }
